  ...
  const handleFetchWeather = async () => {
    ...
    try {
      ...
      if (name.toLowerCase() !== city.toLowerCase()) {
        throw new Error(
          `A cidade retornada (${name}) não corresponde à cidade pesquisada (${city}).`
        );
      }
      ...
      setError(
        "Não foi possível obter informações. Por favor, tente novamente."
      );
      setWeatherData(null);
    } finally {
      setLoading(false);
    ...
    <section className="weather-container">
      <div className="search-weather-content">
        <div className="intro-text">
          <p>
            Hoje precisa de roupa de borracha? Procure o climatempo por cidade e
            estado.
          </p>
        </div>
